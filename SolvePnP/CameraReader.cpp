...
void CameraReader::SetMarkerSize(double LargeMarkerSize) {
    markerDetector.ChangeMarkerSize(LargeMarkerSize, LargeMarkerSize*14/80,
                                    LargeMarkerSize*12/80);
            ...
            for(int j=0;j<count;j++)
            {
                ...
                for (int col = 0; col < 3; col++) {
                    for (int row = 0; row < 3; row++) {
                        srcMat(row, col) = ((Matrix33)tran.r()).mat[row][col];
                    }
                }
                ...
                if(id==0)
                {
                    ...
                    foundMarker.x = tran.t().data[0];
                    foundMarker.y = tran.t().data[1];
                    foundMarker.z = tran.t().data[2];
                    foundMarker.id=0;
                    break;
                }
                else
                {
                    found.radX += dstMat(0, 0);
                    found.radY += dstMat(0, 1);
                    found.radZ += dstMat(0, 2);
                    switch (id) {
                    case 1:
                        found.x += tran.t().data[0];
                        found.y += tran.t().data[1]-27/80*largeMarkerSize;
                        break;
                    case 2:
                        found.x += tran.t().data[0];
                        found.y += tran.t().data[1]-13/80*largeMarkerSize;
                        break;
                    case 3:
                        found.x += tran.t().data[0]-27/80*largeMarkerSize;
                        found.y += tran.t().data[1];
                        break;
                    case 4:
                        found.x += tran.t().data[0]-13/80*largeMarkerSize;
                        found.y += tran.t().data[1];
                        break;
                    case 5:
                        found.x += tran.t().data[0];
                    ...
                    found.z += tran.t().data[2];

                }
            }
            if(tran.id()!=0)
            {
                foundMarker.radX=found.radX/count;
                foundMarker.radY=found.radY/count;
                foundMarker.radZ=found.radZ/count;
                foundMarker.degreeX=found.radX*180/M_PI;
                foundMarker.degreeY=found.radY*180/M_PI;
                foundMarker.degreeZ=found.radZ*180/M_PI;
                foundMarker.x=found.x/count;
                foundMarker.y=found.y/count;
                foundMarker.z=found.z/count;
                foundMarker.id=1234;
            }
