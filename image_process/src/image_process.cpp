...
#include "camerareader/Marker.hpp"
#include "camerareader/MarkerDetector.hpp"
#include "camerareader/CameraCalibration.hpp"
...
//        kc3 = 0,
//        kc4 = 0;
float        distorsionCoeff[4] = {0.0, 0.0, 0.0, 0.0};
...
{
  markerDetector.ChangeMarkerSize(79, 0.945,21.7);
  ...
  frame.height = origin_img.rows;
  frame.width = origin_img.cols;
  frame.stride = origin_img.step;
  frame.data = origin_img.data;
  markerDetector.processFrame(frame);
  trans = markerDetector.getTransformations();
  ...
  for (tr = trans.begin(); tr != trans.end(); tr++) {
      ...
      if(tr.base()->id()!=-1)
      {
          ...
          distx = tr.base()->t().data[0];
          disty = tr.base()->t().data[1];
          distz = tr.base()->t().data[2];

      }
  }

  image_process::Distance dist;
  dist.x_error= distx;//飞机的前向对应图像y轴方向的误差，误差为正则飞机向前方飞行才能减小误差
  dist.y_error= disty;//飞机的右向对应图像x轴方向的误差，误差为正则飞机向右侧飞行才能减小误差
  dist.height= distz;//
  dist.phi = dstMat(0,2)*180/M_PI;
  dist.pix_x = markerDetector.xPoints;
  dist.find_pic = count;
